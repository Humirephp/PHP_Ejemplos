<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Busqueda de Pacientes</title>
    <style>
        body {
            font-family: Arial, sans-serif;
            margin: 0;
            padding: 0;
            background-color: #f2f2f2; /* Color de fondo de la página */
        }
        h1 {
            text-align: center;
            margin-top: 20px;
        }
        table {
            width: 100%;
            border-collapse: collapse;
            margin-top: 20px;
            border: 1px solid black; /* Borde de la tabla */
        }
        th, td {
            border: 1px solid black;
            padding: 8px;
            text-align: left;
        }
        th {
            background-color: #ddd; /* Color de fondo de las celdas de encabezado */
        }
        tr:nth-child(even) {
            background-color: #f2f2f2;
        }
        button {
            padding: 5px 10px;
            background-color: #4CAF50; /* Color de fondo de los botones */
            color: white;
            border: none;
            cursor: pointer;
            margin-right: 5px;
        }
        button:hover {
            background-color: #45a049;
        }
    </style>
</head>
<body>
    
    <center><h1>Busqueda de Pacientes</h1>
    </center>
    <form action="22.1-consulta-pdo.php" method="POST">
    <label for="nombre_apellidos" style="display: inline-block; font-weight: bold;">Nombre o apellidos </label>
    <input type="text"id="nombre_apellidos" placeholder="Escribe aqui" /><br><br>
    <label id="tos" name="tos">Tos</label><input type="checkbox" for="tos">
    <label id="tos" name="fiebre">Fiebre</label><input type="checkbox" for="fiebre">
    <label id="tos" name="disnea">Disnea</label><input type="checkbox" for="disnea">
    <label id="tos" name="Dolor muscular">Dolor muscular</label><input type="checkbox" for="Dolor muscular">
    


    <button type="submit">Buscar</button>
    <table style="border: 1px solid black;">
        <tr>
            <td>Paciente</td>
            <td>Edad</td>
            <td>Talla</td>
            <td>Peso</td>
            <td>Tos</td>
            <td>Fiebre</td>
            <td>Disnea</td>
            <td>Acciones</td>
        </tr>
        <?php 
        if(isset($_GET["consulta"]) ) {
            $pacientes=array(
                0=>array("Nombres"=>"Juan","Edad"=>18,"Talla"=>150,"Peso"=>150,"Tos"=>1,"Fiebre"=>0,"Disnea"=>1),
                1=>array("Nombres"=>"Jose","Edad"=>28,"Talla"=>130,"Peso"=>150,"Tos"=>1,"Fiebre"=>0,"Disnea"=>1),
                2=>array("Nombres"=>"Jose","Edad"=>28,"Talla"=>130,"Peso"=>150,"Tos"=>1,"Fiebre"=>0,"Disnea"=>1),
                3=>array("Nombres"=>"Jose","Edad"=>28,"Talla"=>130,"Peso"=>150,"Tos"=>1,"Fiebre"=>0,"Disnea"=>1),
                4=>array("Nombres"=>"Jose","Edad"=>28,"Talla"=>130,"Peso"=>150,"Tos"=>1,"Fiebre"=>0,"Disnea"=>1),
                5=>array("Nombres"=>"Jose","Edad"=>28,"Talla"=>130,"Peso"=>150,"Tos"=>1,"Fiebre"=>0,"Disnea"=>1),
                6=>array("Nombres"=>"Jose","Edad"=>28,"Talla"=>130,"Peso"=>150,"Tos"=>1,"Fiebre"=>0,"Disnea"=>1)
            );     
            
            foreach ($pacientes as $row) {
                echo "<tr>";
                echo "<td>".$row["Nombres"]."</td>";
                echo "<td>".$row["Edad"]."</td>";
                echo "<td>".$row["Talla"]."</td>";
                echo "<td>".$row["Peso"]."</td>";
                echo "<td>".($row["Tos"]==1 ? "Si" : "No")."</td>";
                echo "<td>".($row["Fiebre"]==1 ? "Si" : "No")."</td>";
                echo "<td>".($row["Disnea"]==1 ? "Si" : "No")."</td>";
                echo "<td> <button>Editar</button><button>Eliminar</button></td>";
                echo "</tr>";
            }
        }
        ?>        
    </table>
    </form>
</body>
</html>
